...
enum BridgeMenuBarActionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingCLI:
            return "Install the global `codex-remote` CLI before using this companion."
...
final class BridgeMenuBarStore: ObservableObject {
    ...
    private static let relayOverrideKey = "codex-remote.menuBar.relayOverride"
    ...
    func openLogsFolder() {
        ...
        let targetPath = (path?.isEmpty == false) ? path! : "\(NSHomeDirectory())/.codex-remote"
