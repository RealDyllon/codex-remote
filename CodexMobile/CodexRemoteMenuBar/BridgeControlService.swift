...
// Purpose: Wraps the existing codex-remote/npm shell commands so the menu bar app can detect the global CLI and control the bridge.
...
struct BridgeCLIInvocation {
    ...
    func command(_ arguments: [String]) -> String {
        ([shellQuoted(nodePath), shellQuoted(codexRemotePath)] + arguments).joined(separator: " ")
    }
}
...
final class BridgeControlService {
    ...
    private let launchAgentPlistURL = URL(fileURLWithPath: NSHomeDirectory())
        ...
        .appendingPathComponent("com.codexremote.bridge.plist")

    init(runner: ShellCommandRunner = ShellCommandRunner()) {
        self.runner = runner
    }

    // Confirms the product contract for this companion: a global `codex-remote` CLI must be runnable first.
    ...
    func updateBridgePackage() async throws {
        _ = try await runner.run(command: "npm install -g codex-remote@latest")
    }

    func fetchLatestPackageVersion() async -> Result<String, Error> {
        do {
            let result = try await runner.run(command: "npm view codex-remote version --json")
            let latestVersion = parseLatestVersion(result.stdout)
            guard let latestVersion else {
                throw BridgeControlError.commandFailed(
                    command: "npm view codex-remote version --json",
    ...
    private func loadFallbackSnapshot(
        ...
        return BridgeSnapshot(
            ...
            label: statusLines["service label"] ?? "com.codexremote.bridge",
            platform: "darwin",
            installed: installed,
    ...
    private func resolveCLIInvocation() async throws -> BridgeCLIInvocation {
        let codexRemotePath = try await resolveExecutable(named: "codex-remote")
        let nodePath = try await resolveNodePath(for: codexRemotePath)
        return BridgeCLIInvocation(nodePath: nodePath, codexRemotePath: codexRemotePath)
    }
    ...
    private func resolveNodePath(for codexRemotePath: String) async throws -> String {
        if let colocatedNodePath = resolveColocatedNodePath(for: codexRemotePath) {
    ...
    private func resolveColocatedNodePath(for codexRemotePath: String) -> String? {
        let codexRemoteURL = URL(fileURLWithPath: codexRemotePath)
        let candidateDirectories = [
            codexRemoteURL.deletingLastPathComponent().path,
            codexRemoteURL.resolvingSymlinksInPath().deletingLastPathComponent().path,
        ]
    ...
    private func resolveStateDirectory(statusLines: [String: String]) -> URL {
        if let explicitStateDirectory = normalizeNonEmptyString(ProcessInfo.processInfo.environment["CODEX_REMOTE_DEVICE_STATE_DIR"]) {
    ...
    private func readLaunchAgentStateDirectory() -> URL? {
        guard let data = try? Data(contentsOf: launchAgentPlistURL),
              ...
              let stateDirectory = normalizeNonEmptyString(environment["CODEX_REMOTE_DEVICE_STATE_DIR"] as? String) else {
            return nil
        }
    ...
    private func classifyCLIAvailability(from error: Error) -> BridgeCLIAvailability {
        ...
        if normalized.contains("command not found: codex-remote")
            || normalized.contains("codex-remote: command not found")
            || normalized.contains("codex-remote: not found")
    ...
    private func commandEnvironment(relayOverride: String?) -> [String: String] {
        ...
        return [
            "CODEX_REMOTE_RELAY": relayOverride.trimmingCharacters(in: .whitespacesAndNewlines),
        ]
    }
}
