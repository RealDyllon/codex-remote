...
enum BridgeCLIAvailability: Equatable {
    ...
    static let installCommand = "npm install -g codex-remote@latest"
    ...
    var setupMessage: String {
        switch self {
        case .checking:
            return "Looking for a globally installed `codex-remote` command before enabling the companion controls."
        case .available(let version):
            return "Detected the global `codex-remote` CLI (`\(version)`)."
        case .missing:
            return "This companion only works when the global `codex-remote` CLI is installed and visible to the app shell environment."
        case .broken(let message):
            return "Found `codex-remote`, but the CLI could not be used. \(message)"
        }
    }
}
