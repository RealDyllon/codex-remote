...
extension CodexService {
    ...
    func performSecureHandshake() async throws {
        ...
        guard serverHello.protocolVersion == codexSecureProtocolVersion else {
            presentBridgeUpdatePrompt(
                message: "This bridge is using a different secure transport version. Update the Codex Remote package on your computer and try again."
            )
            throw CodexSecureTransportError.incompatibleVersion(
                "This bridge is using a different secure transport version. Update Codex Remote on the iPhone or Mac and try again."
            )
        }
    ...
    func secureWireText(for plaintext: String) throws -> String {
        guard var secureSession else {
            throw CodexSecureTransportError.invalidHandshake(
                "The secure Codex Remote session is not ready yet. Try reconnecting."
            )
        }
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw CodexSecureTransportError.invalidHandshake("Unable to encode the secure Codex Remote envelope.")
        }
        return text
    }
...
private extension CodexService {
    ...
    func presentBridgeUpdatePrompt(message: String) {
        bridgeUpdatePrompt = CodexBridgeUpdatePrompt(
            title: "Update the Codex Remote package on your computer",
            message: message,
    ...
    func sendWireControlMessage<Value: Encodable>(_ value: Value) async throws {
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw CodexSecureTransportError.invalidHandshake("Unable to encode the secure Codex Remote control payload.")
        }
    ...
    func waitForSecureControlMessage(kind: String, timeoutSeconds: TimeInterval = 12) async throws -> String {
        ...
        let waiterID = UUID()
        let timeoutMessage = "Timed out waiting for the secure Codex Remote \(kind) message."
    ...
    func handleEncryptedEnvelopeText(_ text: String) {
        ...
        guard let envelope = try? decodeSecureControl(SecureEnvelope.self, from: text),
            ...
            lastErrorMessage = "The secure Codex Remote payload could not be verified."
            secureConnectionState = .rePairRequired
            return
        }
