...
func codexRemoteNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("codex-remote/") {
        return trimmedName
    }
    return "codex-remote/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func codexRemoteCurrentBranchSelectionIsDisabled(
    branch: String,
...
func codexRemoteSelectableDefaultBranch(
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "codex-remote/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = codexRemoteNormalizedCreatedBranchName(searchText)
    ...
    var body: some View {
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = codexRemoteCurrentBranchSelectionIsDisabled(
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = codexRemoteNormalizedCreatedBranchName(searchText)
                        newBranchName = fromSearch.isEmpty ? "codex-remote/" : fromSearch
        ...
        .alert("New branch", isPresented: $isShowingCreateBranchPrompt) {
            TextField("codex-remote/my-feature", text: $newBranchName)
            Button("Cancel", role: .cancel) {
                newBranchName = ""
            }
            Button("Create") {
                let branchName = codexRemoteNormalizedCreatedBranchName(newBranchName)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        branches: [
            "feature/auth-flow",
            "feature/dark-mode",
            "codex-remote/onboarding-v2",
            "codex-remote/sidebar-refactor",
            "fix/crash-on-launch",
        ...
        selectedBranch: "codex-remote/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "codex-remote/onboarding-v2",
