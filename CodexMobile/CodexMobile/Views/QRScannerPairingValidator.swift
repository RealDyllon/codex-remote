...
private let qrScannerBridgeUpdateCommand = "npm install -g codex-remote@latest"
private let qrScannerPairingCodePrefix = "CRX1:"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikeCodexRemotePairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older Codex Remote bridge. Update the npm package on your computer to the latest release before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest Codex Remote bridge.")
}
...
private func looksLikeCodexRemotePairingPayload(_ data: Data) -> Bool {
