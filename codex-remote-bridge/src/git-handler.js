...
async function runStructuredCodexJson({
  ...
  const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-git-ai-"));
...
async function stashChangesForWorktreeHandoff(cwd, pathspecArgs = []) {
  const stashLabel = `codex-remote-worktree-handoff-${randomBytes(6).toString("hex")}`;
  const output = await git(
    cwd,
    "stash",
    "push",
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(os.tmpdir(), `codex-remote-worktree-copy-${randomBytes(6).toString("hex")}.patch`);
...
function normalizeCreatedBranchName(rawName) {
  ...
  if (normalized.startsWith("codex-remote/")) {
    return normalized;
  }
  return `codex-remote/${normalized}`;
}
