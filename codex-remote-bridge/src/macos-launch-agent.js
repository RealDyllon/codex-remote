...
// Purpose: Owns macOS-only launchd install/start/stop/status helpers for the background Codex Remote bridge.
// Layer: CLI helper
// Exports: start/stop/status helpers plus the launchd service runner used by `codex-remote up`.
...
const {
  clearBridgeStatus,
  clearPairingSession,
  ensureCodexRemoteLogsDir,
  ensureCodexRemoteStateDir,
  readBridgeStatus,
  readDaemonConfig,
  readPairingSession,
  resolveBridgeStderrLogPath,
  resolveBridgeStdoutLogPath,
  resolveCodexRemoteStateDir,
  writeBridgeStatus,
  writeDaemonConfig,
  writePairingSession,
} = require("./daemon-state");

const SERVICE_LABEL = "com.codexremote.bridge";
...
function runMacOSBridgeService({ env = process.env } = {}) {
  ...
  if (!config?.relayUrl) {
    ...
    console.error(`[codex-remote] ${message}`);
    return;
  }
...
async function startMacOSBridgeService({
  ...
  ensureCodexRemoteStateDir({ env, fsImpl, osImpl });
  ensureCodexRemoteLogsDir({ env, fsImpl, osImpl });
...
function printMacOSBridgeServiceStatus(options = {}) {
  ...
  console.log(`[codex-remote] Service label: ${status.label}`);
  console.log(`[codex-remote] Installed: ${status.installed ? "yes" : "no"}`);
  console.log(`[codex-remote] Launchd loaded: ${status.launchdLoaded ? "yes" : "no"}`);
  console.log(`[codex-remote] PID: ${status.launchdPid || status.bridgeStatus?.pid || "unknown"}`);
  console.log(`[codex-remote] Bridge state: ${bridgeState}`);
  console.log(`[codex-remote] Connection: ${connectionStatus}`);
  console.log(`[codex-remote] Pairing payload: ${pairingCreatedAt}`);
  console.log(`[codex-remote] Stdout log: ${status.stdoutLogPath}`);
  console.log(`[codex-remote] Stderr log: ${status.stderrLogPath}`);
}
...
function writeLaunchAgentPlist({
  ...
  const stateDir = resolveCodexRemoteStateDir({ env, osImpl });
...
<dict>
  ...
  <dict>
    ...
    <key>CODEX_REMOTE_DEVICE_STATE_DIR</key>
...
function assertRelayConfigured(config) {
  ...
  throw new Error("No relay URL configured. Run ./run-local-codex-remote.sh or set CODEX_REMOTE_RELAY before enabling the macOS bridge service.");
}
