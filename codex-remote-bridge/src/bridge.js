...
const RELAY_HISTORY_IMAGE_REFERENCE_URL = "codex-remote://history-image-elided";
...
function startBridge({
  ...
  if (!relayBaseUrl) {
    console.error("[codex-remote] No relay URL configured.");
    console.error("[codex-remote] In a source checkout, run ./run-local-codex-remote.sh or set CODEX_REMOTE_RELAY.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[codex-remote] ${(error && error.message) || "Failed to load the saved bridge pairing state."}`);
    process.exit(1);
  }
  ...
  codex.onError((error) => {
    ...
    if (config.codexEndpoint) {
      console.error(`[codex-remote] Failed to connect to Codex endpoint: ${config.codexEndpoint}`);
    } else {
      console.error("[codex-remote] Failed to start `codex app-server`.");
      console.error(`[codex-remote] Launch command: ${codex.describe()}`);
      console.error("[codex-remote] Make sure the Codex CLI is installed and that the launcher works on this OS.");
    }
  ...
  function startRelayWatchdog(trackedSocket) {
    ...
    relayWatchdogTimer = setInterval(() => {
      ...
      if (hasRelayConnectionGoneStale(lastRelayActivityAt)) {
        console.warn("[codex-remote] relay heartbeat stalled; forcing reconnect");
  ...
  function logConnectionStatus(status) {
    ...
    console.log(`[codex-remote] ${status}`);
  }
  ...
  function start() {
    ...
    try {
      ...
      nextChild.on?.("error", (error) => {
        consoleImpl.warn(`[codex-remote] Failed to hold the Mac awake while the bridge is active: ${error.message}`);
      });
      nextChild.on?.("exit", () => {
        if (child === nextChild) {
          child = null;
      ...
      consoleImpl.warn(
        `[codex-remote] Failed to start the bridge wake assertion: ${(error && error.message) || "unknown error"}`
      );
      child = null;
    }
  }
