...
class CodexDesktopRefresher {
  ...
  // Keeps one lightweight rollout watcher alive for the current Codex Remote-controlled thread.
...
function readBridgeConfig({
  ...
  const explicitRelayUrl = readFirstDefinedEnv(["CODEX_REMOTE_RELAY"], "", env);
  ...
  const codexEndpoint = readFirstDefinedEnv(
    ["CODEX_REMOTE_CODEX_ENDPOINT"],
    "",
    env
  );
  const refreshCommand = readFirstDefinedEnv(["CODEX_REMOTE_REFRESH_COMMAND"], "", env);
  const explicitRefreshEnabled = readOptionalBooleanEnv(["CODEX_REMOTE_REFRESH_ENABLED"], env);
  const explicitKeepMacAwakeEnabled = readOptionalBooleanEnv(["CODEX_REMOTE_KEEP_MAC_AWAKE"], env);
  ...
  return {
    ...
    pushServiceUrl: readFirstDefinedEnv(
      ["CODEX_REMOTE_PUSH_SERVICE_URL"],
      defaultPushServiceUrl,
      env
    ),
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["CODEX_REMOTE_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160
    ),
    ...
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["CODEX_REMOTE_REFRESH_DEBOUNCE_MS"], String(DEFAULT_DEBOUNCE_MS), env),
      DEFAULT_DEBOUNCE_MS
    ),
    ...
    codexEndpoint,
    refreshCommand,
    codexBundleId: readFirstDefinedEnv(["CODEX_REMOTE_CODEX_BUNDLE_ID"], DEFAULT_BUNDLE_ID, env),
...
function isSourceCheckout(runtimeRoot, fsImpl) {
  ...
  return path.basename(runtimeRoot) === "codex-remote-bridge"
