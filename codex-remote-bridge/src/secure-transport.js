...
const HANDSHAKE_TAG = "codex-remote-e2ee-v1";
    ...
    if (!kind) {
      if (parsed.method || parsed.id != null) {
        sendControlMessage(createSecureError({
          ...
          message: "This bridge requires the latest Codex Remote iPhone app for secure pairing.",
        }));
        return true;
      }
      return false;
...
function debugSecureLog(message) {
  console.log(`[codex-remote][secure] ${message}`);
}
