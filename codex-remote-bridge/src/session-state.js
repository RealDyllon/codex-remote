...
// Purpose: Persists the latest active Codex Remote thread so the user can reopen it on the Mac for handoff.
// Layer: CLI helper
...
const STATE_DIR = path.join(os.homedir(), ".codex-remote");
...
function openLastActiveThread({ bundleId = DEFAULT_BUNDLE_ID } = {}) {
  ...
  if (!threadId) {
    throw new Error("No remembered Codex Remote thread found yet.");
  }
