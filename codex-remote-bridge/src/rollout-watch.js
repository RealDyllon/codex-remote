...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[codex-remote] Watching thread ${resolvedThreadId}`);
  console.log(`[codex-remote] Rollout file: ${rolloutPath}`);
  console.log("[codex-remote] Waiting for new persisted events... (Ctrl+C to stop)");
...
function resolveThreadId(threadId) {
  ...
  throw new Error("No thread id provided and no remembered Codex Remote thread found.");
}
