...
const DEFAULT_STATE_DIR_NAME = ".codex-remote";
...
// Reuses the existing Codex Remote state root so daemon mode keeps the same local-first storage model.
function resolveCodexRemoteStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.CODEX_REMOTE_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveCodexRemoteStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveCodexRemoteStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveCodexRemoteStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveCodexRemoteStateDir(options), LOGS_DIR);
}
...
// Captures the last known service heartbeat so `codex-remote status` does not depend on launchctl output alone.
...
function ensureCodexRemoteStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveCodexRemoteStateDir(options), { recursive: true });
}

function ensureCodexRemoteLogsDir({ fsImpl = fs, ...options } = {}) {
