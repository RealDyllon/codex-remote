...
// Purpose: Detects, installs, and updates the global Codex CLI with transparent logs for Codex Remote installs and startup flows.
// Layer: CLI helper
...
const SKIP_BOOTSTRAP_ENV_NAME = "CODEX_REMOTE_SKIP_CODEX_BOOTSTRAP";
...
function ensureCodexCLI({
  ...
  if (shouldSkipCodexBootstrap(env)) {
    logInfo(logger, `[codex-remote] Skipping Codex CLI bootstrap because ${SKIP_BOOTSTRAP_ENV_NAME}=1.`);
    return {
      status: "skipped",
      versionBefore: null,
      versionAfter: null,
  ...
  logInfo(logger, "[codex-remote] Checking Codex CLI...");
  ...
  if (versionBefore) {
    logInfo(logger, `[codex-remote] Codex CLI found (${versionBefore}).`);
  } else {
    logInfo(logger, "[codex-remote] Codex CLI not found.");
  }
  ...
  if (!npmVersion) {
    logWarn(
      logger,
      "[codex-remote] npm is unavailable, so Codex Remote could not install or update the Codex CLI automatically."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(logger, `[codex-remote] ${actionVerb} Codex CLI via npm (${CODEX_PACKAGE_SPEC})...`);
  ...
  try {
    ...
    logWarn(
      logger,
      `[codex-remote] Codex CLI ${versionBefore ? "update" : "install"} failed. ${message}`
    );
    return {
      status: "failed",
      versionBefore,
  ...
  if (!versionAfter) {
    logWarn(
      logger,
      "[codex-remote] Codex CLI bootstrap finished, but `codex --version` is still unavailable in this shell."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(
    logger,
    `[codex-remote] Codex CLI ${versionBefore ? "updated" : "installed"} (${versionAfter}).`
  );
  return {
