...
const LEGACY_BRIDGE_DOWNGRADE_COMMAND = `npm install -g codex-remote@${LEGACY_BRIDGE_VERSION_FOR_IOS_1_0}`;
...
function buildLegacyIOSAppCompatibilityMessage({
  ...
  return `Codex Remote bridge ${normalizedBridgeVersion} requires Codex Remote iPhone `
    + `${MINIMUM_SUPPORTED_IOS_APP_VERSION} or later. `
    + `Update the iPhone app from the App Store first, or install Codex Remote bridge `
...
function buildCachedIOSAppCompatibilityWarning({
  ...
  return formatNoticeBox({
    title: "!!! WARNING !!!",
    lines: [
      `Codex Remote bridge ${snapshot.bridgeVersion || "latest"} requires Codex Remote iPhone ${snapshot.minimumSupportedIOSAppVersion} or later.`,
