...
test("daemon-state stores config, pairing payloads, and status under the codex-remote state dir", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    assert.equal(resolveCodexRemoteStateDir(), rootDir);
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-daemon-state-"));
