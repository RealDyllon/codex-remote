...
function makeTempRepo() {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-git-handler-"));
  git(repoDir, "init", "-b", "main");
  git(repoDir, "config", "user.name", "Codex Remote Tests");
  git(repoDir, "config", "user.email", "tests@example.com");
  fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n");
  fs.mkdirSync(path.join(repoDir, "codex-remote-bridge", "src"), { recursive: true });
  fs.writeFileSync(path.join(repoDir, "codex-remote-bridge", "src", "index.js"), "export const ready = true;\n");
  git(repoDir, "add", "README.md");
  git(repoDir, "add", "codex-remote-bridge/src/index.js");
...
function makeBareRemote() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-git-handler-remote-"));
}
...
test("gitBranches scopes worktree paths to the current project subdirectory", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "codex-remote-bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  ...
  try {
    ...
    assert.equal(
      ...
      canonicalPath(path.join(siblingWorktree, "codex-remote-bridge"))
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(siblingWorktree, { recursive: true, force: true });
...
test("gitBranches scopes local checkout path for subdirectory worktrees", async () => {
  const repoDir = makeTempRepo();
  const localProjectDir = path.join(repoDir, "codex-remote-bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  const siblingProjectDir = path.join(siblingWorktree, "codex-remote-bridge");
...
test("gitCheckout surfaces a specific error when the requested branch does not exist locally", async () => {
  ...
  try {
    await assert.rejects(
      __test.gitCheckout(repoDir, { branch: "codex-remote/missing" }),
      (error) =>
        error?.errorCode === "branch_not_found"
          && error?.userMessage === "Branch 'codex-remote/missing' does not exist locally."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch normalizes bare names into codex-remote/* and checks out the new branch", async () => {
  const repoDir = makeTempRepo();

  try {
    const result = await __test.gitCreateBranch(repoDir, { name: "new-branch" });

    assert.equal(result.branch, "codex-remote/new-branch");
    assert.equal(result.status?.branch, "codex-remote/new-branch");
    assert.equal(git(repoDir, "rev-parse", "--abbrev-ref", "HEAD"), "codex-remote/new-branch");
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
});

test("normalizeCreatedBranchName avoids double-prefixing codex-remote branches", () => {
  assert.equal(__test.normalizeCreatedBranchName("feature/foo"), "codex-remote/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("codex-remote/feature/foo"), "codex-remote/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("my new branch"), "codex-remote/my-new-branch");
  assert.equal(__test.normalizeCreatedBranchName("feature / login page"), "codex-remote/feature/login-page");
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "invalid_branch_name"
          && error?.userMessage === "Branch 'codex-remote/feature..oops' is not a valid Git branch name."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch rejects duplicate branch names with a specific error", async () => {
  ...
  try {
    git(repoDir, "branch", "codex-remote/already-there");

    await assert.rejects(
      __test.gitCreateBranch(repoDir, { name: "already-there" }),
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'codex-remote/already-there' already exists."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitBranches hides remote-only branches from the local selector list", async () => {
  ...
  try {
    pushRemoteOnlyBranch(repoDir, remoteDir, "codex-remote/remote-only");

    const result = await __test.gitBranches(repoDir);

    assert.ok(!result.branches.includes("codex-remote/remote-only"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
...
test("gitBranches preserves the repo default branch even when it is not checked out locally", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "codex-remote/topic");
    ...
    assert.ok(result.branches.includes("codex-remote/topic"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch rejects names that already exist only on origin", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'codex-remote/remote-only' already exists on origin. Check it out locally instead of creating a new branch."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
...
test("gitStatus marks a branch as published when origin has it even without local upstream", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "codex-remote/published-no-upstream");
...
test("gitGeneratePullRequestDraft summarizes branch changes against the default branch", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "codex-remote/pr-draft");
    fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n\nbranch change\n");
    git(repoDir, "add", "README.md");
    ...
    assert.match(capturedInvocation?.prompt || "", /Current branch: codex-remote\/pr-draft/);
...
test("gitGenerateCommitMessage supports repositories without an initial commit", async () => {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-git-handler-unborn-"));
  let capturedInvocation = null;
...
test("gitCreateWorktree creates a managed worktree under CODEX_HOME/worktrees", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "codex-remote-bridge");
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-codex-home-"));
  ...
  try {
    ...
    assert.equal(result.branch, "codex-remote/new-worktree");
    assert.equal(result.alreadyExisted, false);
    assert.ok(result.worktreePath.startsWith(managedWorktreesRoot));
    assert.equal(path.basename(result.worktreePath), "codex-remote-bridge");
    assert.equal(git(result.worktreePath, "rev-parse", "--abbrev-ref", "HEAD"), "codex-remote/new-worktree");
...
test("gitCreateWorktree reuses an existing worktree for the same codex-remote branch", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "codex-remote-bridge");
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-codex-home-"));
  const previousCodexHome = process.env.CODEX_HOME;
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-codex-remote-existing`);

  process.env.CODEX_HOME = codexHome;

  try {
    git(repoDir, "branch", "codex-remote/existing");
    git(repoDir, "worktree", "add", siblingWorktree, "codex-remote/existing");
    ...
    assert.equal(result.branch, "codex-remote/existing");
    assert.equal(result.alreadyExisted, true);
    assert.equal(result.worktreePath, canonicalPath(path.join(siblingWorktree, "codex-remote-bridge")));
  } finally {
    if (previousCodexHome === undefined) {
      delete process.env.CODEX_HOME;
    } else {
...
test("gitCreateWorktree rejects a reused local branch name before ignoring the chosen base branch", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'codex-remote/already-there' already exists locally. Choose another name or open that branch instead."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateWorktree carries tracked and untracked changes into the new worktree and cleans local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "codex-remote-bridge", "scratch.txt"), "carry me\n");
    ...
    assert.equal(git(repoDir, "status", "--short"), "");
    assert.equal(fs.existsSync(path.join(repoDir, "codex-remote-bridge", "scratch.txt")), false);
...
test("gitCreateWorktree can copy tracked and untracked changes into the new worktree without cleaning local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "codex-remote-bridge", "scratch.txt"), "keep me too\n");
    ...
    assert.match(git(repoDir, "status", "--short"), /codex-remote-bridge\/src\/index\.js/);
    assert.equal(
      fs.readFileSync(path.join(repoDir, "codex-remote-bridge", "scratch.txt"), "utf8"),
      "keep me too\n"
    );
