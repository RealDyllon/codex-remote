...
test("readBridgeConfig keeps safe defaults and explicit overrides", () => {
  const macConfig = readBridgeConfig({
    env: {},
    platform: "darwin",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
      existsSync: () => false,
      readFileSync: () => {
  ...
  const persistedKeepAwakeConfig = readBridgeConfig({
    env: {
      CODEX_REMOTE_DEVICE_STATE_DIR: "/tmp/codex-remote-state",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
      existsSync(targetPath) {
        return targetPath === "/tmp/codex-remote-state/daemon-config.json";
      },
      readFileSync(targetPath) {
        if (targetPath === "/tmp/codex-remote-state/daemon-config.json") {
  ...
  const macEndpointConfig = readBridgeConfig({
    env: { CODEX_REMOTE_CODEX_ENDPOINT: "ws://localhost:8080" },
    platform: "darwin",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const linuxCommandConfig = readBridgeConfig({
    env: { CODEX_REMOTE_REFRESH_COMMAND: "echo refresh" },
    platform: "linux",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const explicitOnConfig = readBridgeConfig({
    env: {
      CODEX_REMOTE_CODEX_ENDPOINT: "ws://localhost:8080",
      CODEX_REMOTE_REFRESH_ENABLED: "true",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
  ...
  const explicitOffConfig = readBridgeConfig({
    env: {
      CODEX_REMOTE_REFRESH_COMMAND: "echo refresh",
      CODEX_REMOTE_REFRESH_ENABLED: "false",
      CODEX_REMOTE_KEEP_MAC_AWAKE: "false",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/codex-remote-package",
    fsImpl: {
...
test("readBridgeConfig uses only the packaged relay default outside a source checkout", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-package-"));
...
test("readBridgeConfig does not use the hosted fallback inside a source checkout", () => {
  const config = readBridgeConfig({
    env: {},
    runtimeRoot: "/workspace/codex-remote-bridge",
    fsImpl: {
      existsSync(targetPath) {
...
test("readBridgeConfig preserves reverse-proxy subpaths when deriving push URLs", () => {
  const config = readBridgeConfig({
    env: {
      CODEX_REMOTE_PUSH_SERVICE_URL: "https://relay.example/codex-remote",
    },
    runtimeRoot: "/workspace/codex-remote-bridge",
    fsImpl: {
      existsSync() {
  ...
  assert.equal(config.pushServiceUrl, "https://relay.example/codex-remote");
});
...
test("readBridgeConfig disables managed push defaults when a self-hosted relay override is set", () => {
  ...
  fs.writeFileSync(
    path.join(srcDir, "private-defaults.json"),
    JSON.stringify({ relayUrl: "wss://relay.example/codex-remote/relay" }),
    "utf8"
  );

  const config = readBridgeConfig({
    env: {
      CODEX_REMOTE_RELAY: "wss://self-host.example/relay",
    },
    runtimeRoot: tempRoot,
    fsImpl: fs,
  });
