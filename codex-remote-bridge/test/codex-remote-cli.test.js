...
// Depends on: node:test, node:assert/strict, child_process, path, ../package.json, ../bin/codex-remote
...
const { main } = require("../bin/codex-remote");

test("codex-remote --version prints the package version", () => {
  const cliPath = path.join(__dirname, "..", "bin", "codex-remote.js");
...
test("codex-remote restart reuses the macOS service start flow", async () => {
  const calls = [];
  const messages = [];

  await main({
    argv: ["node", "codex-remote", "restart"],
    platform: "darwin",
    consoleImpl: {
      log(message) {
    ...
    deps: {
      ...
      async startMacOSBridgeService(options) {
        ...
        return {
          plistPath: "/tmp/codex-remote.plist",
  ...
  assert.deepEqual(messages, [
    "[codex-remote] macOS bridge service restarted.",
  ]);
});

test("codex-remote status --json exposes daemon metadata for companion apps", async () => {
  const writes = [];
  ...
  try {
    await main({
      argv: ["node", "codex-remote", "status", "--json"],
      platform: "darwin",
      consoleImpl: {
        log() {},
        error(message) {
