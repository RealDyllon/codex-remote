...
test("ensureCodexCLI installs Codex when it is missing", () => {
  ...
  assert.deepEqual(messages, [
    "[codex-remote] Checking Codex CLI...",
    "[codex-remote] Codex CLI not found.",
    "[codex-remote] Installing Codex CLI via npm (@openai/codex@latest)...",
    "[codex-remote] Codex CLI installed (0.120.0).",
  ]);
});
...
test("ensureCodexCLI updates Codex when it is already installed", () => {
  ...
  assert.deepEqual(messages, [
    "[codex-remote] Checking Codex CLI...",
    "[codex-remote] Codex CLI found (0.118.0).",
    "[codex-remote] Updating Codex CLI via npm (@openai/codex@latest)...",
    "[codex-remote] Codex CLI updated (0.120.0).",
  ]);
});
...
test("shouldSkipCodexBootstrap respects the opt-out env flag", () => {
  assert.equal(shouldSkipCodexBootstrap({ CODEX_REMOTE_SKIP_CODEX_BOOTSTRAP: "1" }), true);
  assert.equal(shouldSkipCodexBootstrap({ CODEX_REMOTE_SKIP_CODEX_BOOTSTRAP: "true" }), true);
  assert.equal(shouldSkipCodexBootstrap({ CODEX_REMOTE_SKIP_CODEX_BOOTSTRAP: "0" }), false);
});
