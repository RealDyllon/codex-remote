...
test("buildLaunchAgentPlist points launchd at run-service with codex-remote state paths", () => {
  const plist = buildLaunchAgentPlist({
    homeDir: "/Users/tester",
    pathEnv: "/usr/local/bin:/usr/bin",
    stateDir: "/Users/tester/.codex-remote",
    stdoutLogPath: "/Users/tester/.codex-remote/logs/bridge.stdout.log",
    stderrLogPath: "/Users/tester/.codex-remote/logs/bridge.stderr.log",
    nodePath: "/usr/local/bin/node",
    cliPath: "/tmp/codex-remote/bin/codex-remote.js",
  });

  assert.match(plist, /<string>com\.codexremote\.bridge<\/string>/);
  ...
  assert.match(plist, /<key>CODEX_REMOTE_DEVICE_STATE_DIR<\/key>/);
});
...
test("resolveLaunchAgentPlistPath writes into the user's LaunchAgents folder", () => {
  assert.equal(
    ...
    path.join("/Users/tester", "Library", "LaunchAgents", "com.codexremote.bridge.plist")
  );
});
    ...
    assert.deepEqual(calls, [
      [
        ...
        [
          "bootout",
          `gui/${process.getuid()}`,
          path.join(process.env.HOME, "Library", "LaunchAgents", "com.codexremote.bridge.plist"),
        ],
      ],
      [
        "launchctl",
        [
          "bootout",
          `gui/${process.getuid()}/com.codexremote.bridge`,
        ],
      ],
    ]);
  });
...
test("getMacOSBridgeServiceStatus reports launchd + runtime metadata together", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "LaunchAgents", "com.codexremote.bridge.plist");
    fs.mkdirSync(path.dirname(plistPath), { recursive: true });
    fs.writeFileSync(plistPath, "plist");

    const status = getMacOSBridgeServiceStatus({
      platform: "darwin",
      env: { HOME: rootDir, CODEX_REMOTE_DEVICE_STATE_DIR: rootDir },
      execFileSyncImpl() {
        return "pid = 55";
      },
    });
...
function withTempDaemonEnv(run) {
  const previousDir = process.env.CODEX_REMOTE_DEVICE_STATE_DIR;
  const previousHome = process.env.HOME;
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-remote-launch-agent-"));
  process.env.CODEX_REMOTE_DEVICE_STATE_DIR = rootDir;
  ...
  try {
    ...
    if (previousDir === undefined) {
      delete process.env.CODEX_REMOTE_DEVICE_STATE_DIR;
    } else {
      process.env.CODEX_REMOTE_DEVICE_STATE_DIR = previousDir;
    }
