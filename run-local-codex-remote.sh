...
# FILE: run-local-codex-remote.sh
# Purpose: Starts a local relay plus the public bridge for OSS and self-host workflows.
# Layer: developer utility
# Exports: none
# Depends on: node, npm, curl, relay/server.js, codex-remote-bridge/bin/codex-remote.js

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRIDGE_DIR="${ROOT_DIR}/codex-remote-bridge"
...
log() {
  echo "[run-local-codex-remote] $*"
}

die() {
  echo "[run-local-codex-remote] $*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: ./run-local-codex-remote.sh [options]
...
print_summary() {
  cat <<EOF
[run-local-codex-remote] Configuration
...
start_bridge() {
  ...
  CODEX_REMOTE_RELAY="ws://${RELAY_HOSTNAME}:${RELAY_PORT}/relay" node ./bin/codex-remote.js run &
  BRIDGE_PID=$!
}

hold_open() {
  log "Local relay is ready. Keep this terminal open while testing."
  log "Press Ctrl+C to stop both the local relay and the Codex Remote bridge service."
  while true; do
    if [[ -n "${RELAY_PID}" ]] && ! kill -0 "${RELAY_PID}" 2>/dev/null; then
      wait "${RELAY_PID}"
      return $?
